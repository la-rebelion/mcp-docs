...
Interactive OAuth2 — the flow this guide walks through — is a licensed
feature in v1: it needs the `@mcp-com-ai/enterprise-plugin-auth` plugin and a
`core.enterprise`/`auth.oauth-broker` entitlement installed alongside the
free, open-source HAPI CLI. See [What's New in v1: Authentication & Licensing](/introduction/whats-new-in-v1#oauth-is-now-a-licensed-enterprise-feature)
