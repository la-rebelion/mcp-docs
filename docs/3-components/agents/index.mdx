...
<InlineSVG name="agent-architecture-diagram"
    alt="HAPI Stack Components Overview"
    width="100%"
...
Agents in the HAPI Stack implement **Context Engineering** (see [ChatMCP Context Engineering](//components/chatmcp#Context-Engineering-in-chatMCP)) to ensure every chat flow is:
...
<!-- @todo We don't want the users to code, for users interaction is through the UI, so we don't need to show code examples here
```ts
...
- [ChatMCP: Conversational AI](/components/chatmcp/)
- [Chat Flow Design](/components/chatmcp/chat-flow-design)
